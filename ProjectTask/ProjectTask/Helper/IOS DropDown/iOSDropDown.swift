...
open class iOSDropDown : UITextField{
    ...
    fileprivate  var dataDictArray = [NSDictionary]()
    ...
    public var optionDictArray = [NSDictionary]() {
        didSet{
            self.dataDictArray = self.optionDictArray
        }
    }
    ...
    fileprivate var didSelectCompletionDict: (NSDictionary, Int ,Int) -> () = {selectedText, index , id  in }
    ...
    public func showList()
    {
        ...
        {
            if listHeight > rowHeight * CGFloat( dataDictArray.count)
            {
                self.tableheightX = rowHeight * CGFloat(dataDictArray.count)
            }
            else
            {
        ...
        table = UITableView(frame: CGRect(x: pointToParent.x ,
                                          ...
                                          width: self.frame.width-(pointToParent.x*2),
    ...
    public func didSelectDict(completion: @escaping (_ selectedText: NSDictionary, _ index: Int , _ id:Int ) -> ()) {
        didSelectCompletionDict = completion
    }
...
extension iOSDropDown : UITextFieldDelegate {
...
extension iOSDropDown: UITableViewDataSource
{
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if dataArray.count == 0
        {
            let dictCell = dataDictArray[indexPath.row]
            
            cell!.textLabel!.textColor = rowTextColor
            cell!.textLabel!.text = "\(dictCell.value(forKey: key as String) ?? "")"
...
extension iOSDropDown: UITableViewDelegate {
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath)
    {
        if dataArray.count == 0
        {
            ...
            let selectedDict = self.dataDictArray[self.selectedIndex!]
            ...
            UIView.animate(withDuration: 0.5,
                           ...
                           completion: { (didFinish) -> Void in
                            self.text = "\(selectedDict.value(forKey: self.key as String) ?? "")"
            ...
            if let selected = optionDictArray.index(where: {$0 == selectedDict}) {
                if let id = optionIds?[selected] {
                    didSelectCompletionDict(selectedDict, selected , id )
                }else{
                    didSelectCompletionDict(selectedDict, selected , 0)
                }
            }
